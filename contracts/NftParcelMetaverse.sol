...
contract NftParcelMetaverse is ERC721, OnChainMetadata {
    ...
    bytes32 constant key_token_name_world = "nameWorld";
    bytes32 constant key_token_size = "size";
    bytes32 constant key_token_type = "type";
    bytes32 constant key_token_map = "map";
    constructor(string memory name, string memory symbol, string memory _image) 
        ERC721(name, symbol) {
    ...
    function claimNftParcel(string memory _nameWorld, string memory _size, string memory _type, string memory _map) public {

        _setValue(tokenCounter, key_token_name, abi.encode(string(abi.encodePacked(name(), ' #', Strings.toString(tokenCounter)))));
        _setValue(tokenCounter, key_token_description, abi.encode("Testing NFT for minting metaverse land and i hope all the metadata can see in open fucking sea"));
        _setValue(tokenCounter, key_token_image, abi.encode(image));
        _setValue(tokenCounter, key_token_name_world, abi.encode(_nameWorld));
        ...
        trait_types[0] = abi.encode("nameWorld");
        trait_types[1] = abi.encode("size");
        trait_types[2] = abi.encode("type");
        trait_types[3] = abi.encode("map");
        trait_values[0] = abi.encode(_nameWorld);
        trait_values[1] = abi.encode(_size);
        trait_values[2] = abi.encode(_type);
        trait_values[3] = abi.encode(_map);
